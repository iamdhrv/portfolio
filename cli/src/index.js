...
class LoadingScreen {
  constructor(screen, callback) {
    ...
    this.progress = 0;
    this.bars = [
      { name: 'Loading assets', speed: 30 },
      { name: 'Initializing', speed: 40 },
      { name: 'Building UI', speed: 35 },
      { name: 'Loading content', speed: 25 },
      { name: 'Ready!', speed: 50 }
    ];
    this.currentBar = 0;
    this.charIndex = 0;
  ...
  createUI() {
    ...
    this.status = blessed.box({
      ...
      left: 'center',
      width: '60%',
      content: '{cyan}Initializing...{white}',
      style: { fg: theme.fg }
    });
  ...
  async animateBar(bar) {
    ...
    while (progress <= width) {
      ...
      const barStr = `{cyan}${bar.name}{white}:\n[${theme.highlight}${filledStr}${theme.dim}${emptyStr}{white}] ${percent}%`;
...
class PortfolioApp {
  ...
  async typeText(text, delay = 15) {
    let result = '';
    for (let i = 0; i < text.length; i++) {
      result += text[i];
      this.content.setContent(result + '{blink}__{/blink}');
      this.screen.render();
      await this.sleep(delay + Math.random() * 10);
    }
    // Remove blinking cursor at end
    this.content.setContent(result);
    this.screen.render();
  }
    ...
    if (animate) {
      ...
      for (const line of section.lines) {
        ...
        for (let i = 0; i <= line.length; i++) {
          ...
          // Rebuild content with typed line
          let newContent = content;
          const existingLines = section.lines.indexOf(line);
          for (let j = 0; j < existingLines; j++) {
