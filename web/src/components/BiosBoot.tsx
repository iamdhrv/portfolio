...
export default function BiosBoot({ onComplete }: Props) {
  ...
  if (stage === "done") return <div className="fixed inset-0 bg-black animate-pulse" />;

  return (
    <div className="fixed inset-0 bg-zinc-950 font-mono text-sm md:text-base overflow-hidden">
      <div className="pointer-events-none absolute inset-0 z-50">
        <div className="absolute inset-0 bg-[linear-gradient(rgba(18,16,16,0)_50%,rgba(0,0,0,0.15)_50%)]" style={{ backgroundSize: "100% 2px" }} />
      </div>
      {stage === "logo" ? (
        <div className="h-full flex flex-col items-center justify-center animate-pulse">
          <div className="text-cyan-400 text-4xl md:text-6xl font-bold mb-8 tracking-wider">DhruvOS</div>
          <div className="text-zinc-500 text-xl">Building the future...</div>
          <div className="mt-4 text-zinc-600 text-sm">Version 1.0.0</div>
        </div>
      ) : (
        <div className="p-4 md:p-8 max-w-3xl">
          {lines.map((line, i) => (
            <div key={i} className="leading-relaxed">
              ...
              {i === lines.length - 1 && showCursor && <span className="text-cyan-400 animate-pulse">▋</span>}
            </div>
          ))}
          {stage !== "logo" && showCursor && <div className="text-cyan-400 animate-pulse">▋</div>}
        </div>
      )}
      <div className="fixed bottom-2 right-4 text-zinc-600 text-xs">BIOS Date: 03/12/26 14:22:51 Ver: 1.0.0</div>
    </div>
  );
}
